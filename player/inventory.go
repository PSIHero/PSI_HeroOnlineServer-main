...
func (gih *GetInventoryHandler) Handle(s *database.Socket) ([]byte, error) {
	...
	if inventory[0x0A].ItemID > 0 { // pet
		showpet, _ := s.Character.ShowItems()
		resp.Concat(showpet)
	}

	// if s.Character.DoesInventoryExpanded() { //Quoted out because there is no such item in use yet
...
func (h *BuyHTItemHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if item, ok := database.HTItems[itemID]; ok && item.IsActive && s.User.NCash >= uint64(item.Cash) {
		...
		text := fmt.Sprintf("Name: "+s.Character.Name+"("+s.Character.UserID+") bought: "+iteminfo.Name+" for (%d)ncash.", database.HTItems[int(itemID)].Cash)
...
func (h *ReplaceHTItemHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	_, ok := database.GetItemInfo(item.ItemID)
	if !ok {
		return nil, nil
	}
...
func (h *InspectItemHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	for _, s := range slots {
		...
		index += 15
		resp.Insert([]byte{0x00, 0x00, 0x00}, index) //SOCKET LATER MUST FIX
		index += 3
	}
...
func (h *DressUpHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if isHT {
		...
		ITEM_DRESS_DOWN := utils.Packet{0xaa, 0x55, 0x06, 0x00, 0x59, 0x11, 0x0a, 0x00, 0x02, 0x01, 0x55, 0xaa}
		resp := ITEM_DRESS_DOWN
		resp[8] = data[6]
		resp[9] = data[7]
		itemsData, err := s.Character.ShowItems()
		if err != nil {
...
func (h *MaterialDestructionHandler) Handle(s *database.Socket, data []byte) []byte {
	return utils.Packet{0xAA, 0x55, 0x04, 0x00, 0x54, 0x13, 0x0A, 0x00, 0x55, 0xAA}
}
...
func (h *UseConsumableHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if info.Type == 93 {
		dungeon.MobScrollUse(info.NPCID, s.Character)
	}

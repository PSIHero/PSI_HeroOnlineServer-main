...
func createMobs() {
	...
	for _, action := range posID {
		...
		for i := 0; i < int(npcPos.Count); i++ {
			...
			newai := &database.AI{ID: len(database.AIs), HP: npc.MaxHp, Map: npcPos.MapID, PosID: npcPos.ID, RunningSpeed: 10, Server: 1 + 2 + 3 + 4 + 5 + 6 + 7, WalkingSpeed: 5, Once: true}
...
func Init() {
	...
	func() {
		...
		for _, AI := range database.AIs {
			...
			npc, ok := database.NPCs[pos.NPCID]
			if !ok {
				continue
			}
			...
			npcID := []int{424101, 424102, 424103, 424104, 424105, 424203, 424204, 424205, 424206, 424207}
			server.GenerateIDForAI(AI)
			if funk.Contains(npcID, pos.NPCID) {

...
func (h *CoProductionHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	prodCost := uint64(production.Cost)

	if s.Character.Gold < prodCost {
		return nil, nil
	}
	for i := 0; i < len(prodMaterials); i++ {
		if prodMaterials[i] == 0 || prodQty[i] == 0 {
			continue
		}
		matItemId := int64(prodMaterials[i])
		matCount := uint(prodQty[i])

		slotID, _, _ := s.Character.FindItemInInventoryForProduction(nil, matItemId)
		if slotID == -1 {
			return nil, nil
		}

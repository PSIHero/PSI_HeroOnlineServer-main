...
func (h *RegisterItemHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	if s.User.UserType == server.GM_USER || s.User.UserType == server.GAL_USER || s.User.UserType == server.GA_USER || s.User.UserType == server.GM_USER {
		return nil, nil
	}
	...
	if slotID == 0 || item == nil || !itemInfo.Tradable {
		return nil, nil
	}

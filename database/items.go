...
var (
	...
	STRRates      = []int{350, 335, 325, 275, 265, 250, 240, 210, 100, 75, 50, 40, 30, 20, 15}
...
func (item *Item) GetType() int {
	if item.Type == 51 {
		...
		return PENDENT_TYPE
	} else if item.Type == 198 {
		return STORAGE_EXPANSION_TYPE
		...
		return TRANSFORMATION_SCROLL_TYPE
	}
	return UNKNOWN_TYPE
}
...
func (item *Item) CanUse(t byte) bool {
	if item.Type == int16(t) || t == 0 || t == 1 {
		...
		return true
	} else if (item.Type == 44 || item.Type == 49) && (t == 5 || t == 6) { //BEAST
		return true
	}

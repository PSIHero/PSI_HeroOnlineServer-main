...
type User struct {
	...
	PatreonTier        int8      `db:"patreon_tier" json:"patreontier"`
...
func FindUserByAnything(userIdentification string) (*User, error) {
	UserByID, _ := FindUserByID(userIdentification)
	if UserByID != nil {
		return UserByID, nil
	}
	UserByChar, _ := FindCharacterByName(userIdentification)
	if UserByChar != nil {
		UserID, _ := FindUserByID(UserByChar.UserID)
		return UserID, nil
	}
	UserByName, err := FindUserByName(userIdentification)
	if UserByName != nil {
...
func (u *User) GetTime() []byte {
	...
	serverName := fmt.Sprintf("PSI %d", u.ConnectedServer)

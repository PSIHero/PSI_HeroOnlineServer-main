...
var (
	...
	eventBosses     = []int{50009, 50010, 50013, 50015}
	EventItems      = []int{}
	EventProb       = 20
...
func (ai *AI) FindTargetCharacterID() (int, error) {
	...
	filtered := funk.Filter(allChars, func(c *Character) bool {
		...
		//seed := utils.RandInt(0, 1000) // Quoted out for now to make NPCs more responsive
...
func (ai *AI) Attack() []byte {
	...
	if !character.IsOnline || character.Socket == nil {
		return nil
	}
...
func (ai *AI) DealDamage(damage int) []byte {
	...
	character.Injury += 0.1
	/*injuryNumbers := character.CalculateInjury()
	...
	index += 4
	index += 12
	resp.Insert([]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, index) // INJURY
...
func (ai *AI) DealDamageToPet(damage int) []byte {
	...
	resp.Insert([]byte{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}, 15) // INJURY
	resp.SetLength(0x24)
...
func (ai *AI) DropHandler(claimer *Character) {
	...
	isEventBoss := true
	bossMultiplier, dropCount, count, minCount, probabmax := 0.0, 0, 0, 0, 1000
...
BEGIN:
	...
	for ok {
		index := 0
		seed := int(utils.RandInt(0, int64(probabmax)))
		items := drop.GetItems()
		...
		if len(EventItems) > 0 {
			for _, event := range EventItems {
				...
				probabilities = append(probabilities, lastProb)
				if probabilities[len(probabilities)-1] >= 1000 {
					probabmax += EventProb
				}
			}
		}
	...
	if itemID > 0 && !end { // can drop an item
		...
		go func() {
			...
			if item != nil {
				...
				plus := byte(0)
				for i := 0; i < len(plusRates) && !isRelic && drop.ItemType != 1; i++ {
					if seed > plusRates[i] {
						plus++
						continue
					}
				...
				if isRelic || !npcPos.Attackable {
					...
					pet := petSlot.Pet
					if pet != nil {
						if claimer.IsOnline && pet.IsOnline && pet.CombatPet && (petSlot.ItemID == 50093 || petSlot.ItemID == 17504256 || petSlot.ItemID == 17504258 || petSlot.ItemID == 17504263 || petSlot.ItemID == 17807200 || petSlot.ItemID == 17807201 || petSlot.ItemID == 17807568 || petSlot.ItemID == 17807569) && dr.Item.Plus >= 1 {
							testp := utils.Packet{0xaa, 0x55, 0x00, 0x00, 0x00, 0x00, 0x55, 0x0aa}
							testp.Insert(utils.IntToBytes(uint64(dropID), 2, true), 7) // item id
							//LootPetHandler(claimer.Socket, testp)
						}
					}
					/*time.AfterFunc(FREEDROP_LIFETIME, func() { //ALL PLAYER CAN PICKUP THE ITEMS
						_, ok := DropRegister[dr.Server][dr.Map][dropID]
						if ok {
							dr.Claimer = nil
							characters, err := dr.GetNearbyCharactersFromDrop()
							if err != nil {
								log.Println(err)
								return
							}
							...
							for _, chars := range characters {
								...
								/*	r = ITEM_DROPPED
									...
									if drop.Plus > 0 {
										...
										r.Insert(utils.IntToBytes(uint64(chars.PseudoID), 2, true), 66)   // claimer id
										r.SetLength(0x42)
									} else {
										r[27] = 0xA1
										r.Insert(utils.IntToBytes(uint64(chars.PseudoID), 2, true), 36) // claimer id
										r.SetLength(0x24)
									}
							}
						}
...
func (ai *AI) AIHandler() {
	...
	if len(ai.OnSightPlayers) > 0 && ai.HP > 0 {
		...
		if ai.TargetPlayerID == 0 && ai.TargetPetID == 0 { // Idle mode
			...
			character, err := FindCharacterByID(ai.TargetPlayerID)
			if err != nil || character == nil || (character != nil && !character.IsOnline) || character.IsMounting {
				ai.HP = npc.MaxHp
				ai.TargetPlayerID = 0
				goto OUT
			}

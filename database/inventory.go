...
func (slot *InventorySlot) GetPetStats(c *Character) []byte {
	...
	maxCHI := petInfo.BaseChi + levelDiff*int(petInfo.AdditionalChi)
...
func (pet *PetSlot) PlayerAttack(owner *Character) []byte {
	...
	if damage > 0 {
		...
		time.AfterFunc(time.Second/2, func() {
			owner.DealDamageToPlayer(mob, damage, false, false)
		})
	}
...
func (pet *PetSlot) Attack(owner *Character) []byte {
	...
	pet.AddExp(owner, 0)
	time.AfterFunc(time.Second/2, func() {
		owner.DealDamage(mob, damage, false, false)
	})
...
func (pet *PetSlot) CastSkill(owner *Character, skillID int) []byte {
	...
	if skillInfo.BaseMinHP > 0 {
		char := FindCharacterByPseudoID(pet.PetOwner.Socket.User.ConnectedServer, uint16(pet.PetOwner.Selection))
		if char != nil {
			if pet.PetOwner.CanAttack(char) {
				pet.PetOwner.HealPlayer(skillInfo.BaseMinHP)
			} else {
				char.HealPlayer(skillInfo.BaseMinHP)
			}
	...
	for _, target := range candidates {
		...
		pet.AddExp(owner, 0)
		owner.DealDamage(target, damage, true, false)
	}
	//})
...
func (pet *PetSlot) CastSkillToPlayer(owner *Character, skillID int) []byte {
	char := FindCharacterByPseudoID(pet.PetOwner.Socket.User.ConnectedServer, uint16(pet.PetOwner.Selection))
	if char == nil || char.Socket.Stats.HP <= 0 {
		char = pet.PetOwner
	}
	...
	resp.Insert(utils.FloatToBytes(pet.Coordinate.X, 4, true), 13)    // pet-x
	resp.Insert(utils.FloatToBytes(pet.Coordinate.Y, 4, true), 17)    // pet-x
	resp.Insert(utils.IntToBytes(uint64(char.PseudoID), 2, true), 25) // target pseudo id
	resp.Insert(utils.IntToBytes(uint64(char.PseudoID), 2, true), 28) // target pseudo id

	pet.Casting = true
	pet.CHI -= skillInfo.BaseChi
	if skillInfo.InfectionID != 0 {
		duration := (skillInfo.BaseTime + skillInfo.AdditionalTime*int(1)) / 10
		infection := BuffInfections[skillInfo.InfectionID]
		plus := 1
		//buffinfo := BuffInfections[int(skillInfo.InfectionID)]
		buff, err := FindBuffByCharacter(infection.ID, char.ID)
		if err != nil {
			return nil
		} else if buff != nil {
			buff.Delete()
			stat, err := FindStatByID(char.ID)
			if err != nil {
				return nil
			}
			...
			if !infection.IsPercent {
				buff = &Buff{ID: infection.ID, CharacterID: char.ID, StartedAt: char.Epoch, Duration: int64(duration), Name: skillInfo.Name,
					...
					Accuracy: infection.Accuracy + infection.AdditionalAccuracy*int(plus), Dodge: infection.DodgeRate + infection.AdditionalDodgeRate*int(plus), RunningSpeed: infection.MovSpeed + infection.AdditionalMovSpeed*float64(plus), CanExpire: true}
				buff.Create()
			} else {
				percentArtsDEF := int(float64(char.Socket.Stats.ArtsDEF) * (float64(infection.ArtsDEF+infection.AdditionalArtsDEF*int(plus)) / 1000))
				percentDEF := int(float64(char.Socket.Stats.DEF) * (float64(infection.BaseDef+infection.AdditionalDEF*int(plus)) / 1000))
				percentATK := int(float64(char.Socket.Stats.MinATK) * (float64(infection.BaseATK+infection.AdditionalATK*int(plus)) / 1000))
				percentArtsATK := int(float64(char.Socket.Stats.MinArtsATK) * (float64(infection.BaseArtsATK+infection.AdditionalArtsATK*int(plus)) / 1000))
				buff = &Buff{ID: infection.ID, CharacterID: char.ID, StartedAt: char.Epoch, Duration: int64(duration), Name: skillInfo.Name,
					...
					MaxHP: infection.MaxHP + infection.AdditionalHP*int(plus), ParalysisDEF: infection.ParalysisDef + infection.AdditionalParalysisDef*int(plus), PoisonDEF: infection.ParalysisDef + infection.AdditionalPoisonDef*int(plus), STR: infection.STR + infection.AdditionalSTR*int(plus),
					Accuracy: infection.Accuracy + infection.AdditionalAccuracy*int(plus), Dodge: infection.DodgeRate + infection.AdditionalDodgeRate*int(plus), CanExpire: true}
				buff.Create()
			}
			buff.Update()
		...
		statData, _ := char.GetStats()
		char.Socket.Write(statData)

		p := &nats.CastPacket{CastNear: true, CharacterID: char.ID, Data: char.GetHPandChi()}
		p.Cast()
	}
	resp.Concat(petSlot.GetData(0x0A))
	return resp
